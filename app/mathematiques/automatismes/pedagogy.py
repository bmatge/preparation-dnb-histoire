"""Orchestration pédagogique de l'épreuve « Automatismes ».

Trois fonctions publiques :

- `evaluate_answer(question, student_answer)` : dispatch sur le mode de
  scoring de la question. `python` → `scoring.check` (déterministe, pas
  d'Albert). `albert` → `_safe_chat(MATH_AUTO_EVAL_OPEN, ...)` qui
  retourne `True/False` après parsing JSON, avec fallback déterministe
  texte si Albert plante.
...
- `generate_hint(question, hint_level, previous_answers)` : indice
  ...
  type de réponse / la première lettre de la réponse canonique.

- `reveal_answer(question)` : message de révélation via Albert. Fallback
  déterministe avec la réponse canonique brute.
...
from app.mathematiques.automatismes import scoring as auto_scoring
from app.mathematiques.automatismes.prompts import (
    build_hint_prompt,
...
def _safe_chat(task: Task, messages: list[dict], fallback: str) -> str:
    """Wrapper unique pour les appels Albert côté automatismes."""
    try:
...
def evaluate_answer(
    ...
    if mode == "python":
        return auto_scoring.check(scoring, student_answer)
...
def _evaluate_open(
    ...
    try:
        ...
        rag_passages = rag.search_for_task(
            ...
            task=Task.MATH_AUTO_EVAL_OPEN,
            query=question.enonce,
            limit=3,
        )
    except Exception:
        logger.exception("RAG indisponible pour eval ouverte")
    ...
    if parsed is None:
        logger.warning(
            "Réponse Albert non parsable pour eval ouverte (question=%s) : %r",
            question.id,
            raw[:200],
        )
        return False
...
def _try_parse_eval_json(raw: str) -> dict | None:
    ...
    Tolérant : on tente d'abord le parse complet, puis on cherche un
    bloc `{...}` dans la chaîne (au cas où le modèle ait préfixé du
...
def _fallback_hint(question: auto_models.AutoQuestion, hint_level: int) -> str:
    """Indice déterministe utilisé quand Albert est indisponible.

    Niveau 1 : rappelle le thème.
    Niveau 2 : rappelle le type de réponse attendu.
    Niveau 3 : donne le premier caractère de la réponse canonique
               (ou un encadrement très grossier si on l'a).
    """
    ...
    if hint_level == 2:
        ...
        return "Reprends la consigne et identifie ce qu'on te demande."
...
def reveal_answer(question: auto_models.AutoQuestion) -> str:
...
def _fallback_reveal(question: auto_models.AutoQuestion) -> str:
    scoring = question.scoring or {}
    rep = scoring.get("reponse_canonique") or scoring.get("reponse_modele") or "?"
